...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    ...
    var res: Map[Char,Int] = Map[Char, Int]()
    for (c <- chars) {
      res += ((c,res.getOrElse (c, 0)+1))
    }
    res.toList
  }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    var resultSorted = freqs.sortWith(_._2 < _._2)
    var res = new ListBuffer[Leaf]()
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def aux(treeVar: CodeTree, bits: List[Bit]): List[Char] = treeVar match {
      case Leaf(c, _) if bits.isEmpty => List(c)
      case Leaf(c, _) => c :: aux(tree, bits)
      case Fork(left, right, _, _) if bits.head == 0 => aux(left, bits.tail)
      case Fork(left, right, _, _) => aux(right, bits.tail)
    }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def aux(tree:  CodeTree)(c: Char): List[Bit] = tree match {
      case Leaf(_, _) => List()
      case Fork(left, right, _, _) if chars(left).contains(c) => 0 :: aux(left)(c)
      case Fork(left, right, _, _) => 1 :: aux(right)(c)
    }
    // flatMap
    // applique la fonction aux(tree) sur chaque element text
    text flatMap aux(tree)
  }
